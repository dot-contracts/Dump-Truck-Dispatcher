...
namespace DispatcherWeb.Tests.Authorization.Users
{
    public class UserAppService_Link_Tests : UserAppServiceTestBase
    {
        ...
        public UserAppService_Link_Tests()
        {
            ...
            _encryptionService = Resolve<IEncryptionService>();
        }
        ...
        public async Task Should_Link_User_To_Different_Tenant_User()
        {
            ...
            //Assert
            var defaultTenantAdmin = await UsingDbContextAsync(context => context.Users.FirstOrDefaultAsync(u => u.TenantId == Session.TenantId && u.Id == Session.UserId));
        ...
        public async Task Should_Link_User_To_Already_Linked_User()
        {
            ...
            //Assert
            var defaultTenantAdmin = await UsingDbContextAsync(context => context.Users.FirstOrDefaultAsync(u => u.TenantId == Session.TenantId && u.UserName == User.AdminUserName));
        ...
        private async Task<int> CreateTestTenantAndTestUser()
        {
            ...
            {
                ConnectionString = _encryptionService.EncryptIfNotEmpty("Server=localhost; Database=DispatcherWebTest_" + Guid.NewGuid().ToString("N") + "; Trusted_Connection=True;"),
            };
        ...
        private async Task LinkUserAndTestAsync(string tenancyName)
        {
            ...
            var currentUser = await UsingDbContextAsync(context => context.Users.FirstOrDefaultAsync(u => u.Id == Session.UserId));
