...
namespace DispatcherWeb.Tests.Organizations
{
    public class OrganizationUnitAppService_Tests : AppTestBase
    {
        ...
        public async Task Test_DeleteOrganizationUnit()
        {
            ...
            UsingDbContext(context =>
            {
                context.Users.FirstOrDefault(u => u.Id == Session.UserId.Value && u.TenantId == Session.TenantId.Value).ShouldNotBeNull();
            });
        ...
        public async Task Test_RemoveFromOrganizationUnit()
        {
            ...
            await UnitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                ...
                context.UserOrganizationUnits.Add(new UserOrganizationUnit(Session.TenantId, admin.Id, ou12.Id));
            });
        ...
        {
            var organizationUnit = await UnitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                var context = await UnitOfWorkManager.Current.GetDbContextAsync<DispatcherWebDbContext>();
                return await context.OrganizationUnits.FirstOrDefaultAsync(ou => ou.DisplayName == diplayName);
            });
