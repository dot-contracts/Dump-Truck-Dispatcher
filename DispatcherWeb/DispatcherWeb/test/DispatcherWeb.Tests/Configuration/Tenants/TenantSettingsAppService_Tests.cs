...
namespace DispatcherWeb.Tests.Configuration.Tenants
{
    public class TenantSettingsAppService_Tests : AppTestBase
    {
        ...
        public async Task Should_Change_UserManagement_Settings()
        {
            ...
            //Assert
            (await _settingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.AllowSelfRegistration)).ShouldBe(true);
            (await _settingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.IsNewRegisteredUserActiveByDefault)).ShouldBe(true);
            (await _settingManager.GetSettingValueAsync<bool>(AppSettings.UserManagement.UseCaptchaOnRegistration)).ShouldBe(false);
        }
    }
}
