...
{
    public class InvoicePrintOutGenerator7 : ITransientDependency
    {
        private readonly ISettingManager _settingManager;

        public InvoicePrintOutGenerator7(ISettingManager settingManager)
        {
            ...
            foreach (var model in modelList)
            {
                ...
                if (model.InvoiceLines.Any())
                {
                    ...
                    foreach (var invoiceLine in model.InvoiceLines)
                    {
                        ...
                        cell = row.Cells[i++];
                        paragraph = cell.AddParagraph(
                            (invoiceLine.ChargeId.HasValue ? (invoiceLine.ItemName + " ") : "")
                            + invoiceLine.Description, tm);
                        cell = row.Cells[i++];
