...
{
    public class InvoicePrintOutGenerator8 : ITransientDependency
    {
        public InvoicePrintOutGenerator8()
        {
        }
            ...
            foreach (var model in modelList)
            {
                ...
                if (model.InvoiceLines.Any())
                {
                    ...
                    foreach (var group in groupedInvoiceLines)
                    {
                        ...
                        paragraph = cell.AddParagraph($"Ref#: {group.DeliverToStreetAddress ?? ""}");
