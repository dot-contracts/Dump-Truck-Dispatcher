...
namespace DispatcherWeb.DashboardCustomization
{
    ...
    public class DashboardCustomizationAppService : DispatcherWebAppServiceBase, IDashboardCustomizationAppService
    {
        ...
        public async Task<DashboardOutput> GetDashboardDefinition(GetDashboardInput input)
        {
            ...
            var usedWidgetDefinitions = await GetFilteredWidgetsAsync(dashboardDefinition);
        ...
        public async Task<List<WidgetOutput>> GetAllWidgetDefinitions(GetDashboardInput input)
        {
            var dashboardDefinition = _dashboardConfiguration.DashboardDefinitions.FirstOrDefault(d => d.Name == input.DashboardName);
            if (dashboardDefinition == null)
            {
            ...
            return (await GetFilteredWidgetsAsync(dashboardDefinition))
        ...
        private async Task SaveSetting(string application, List<Dashboard> dashboards)
        {
            ...
            await SettingManager.ChangeSettingForUserAsync(await Session.ToUserIdentifierAsync(), GetSettingName(application), value);
        }
        ...
        private async Task<List<Dashboard>> GetDefaultDashboardValue(string application)
        {
            ...
            {
                dashboardConfigAsJsonString = await SettingManager.GetSettingValueForTenantAsync(GetSettingName(application), await AbpSession.GetTenantIdAsync());
            }
        ...
        private async Task<List<WidgetDefinition>> GetFilteredWidgetsAsync(DashboardDefinition dashboardDefinition)
        {
            ...
            return await FilterWidgetsByPermissionAndMultiTenancySide(await FilterWidgetsByMultiTenancySideAsync(widgetDefinitions));
        }

        private async Task<List<WidgetDefinition>> FilterWidgetsByPermissionAndMultiTenancySide(List<WidgetDefinition> widgets)
        {
            return await FilterWidgetsByPermission(await FilterWidgetsByMultiTenancySideAsync(widgets));
        }

        private async Task<List<WidgetDefinition>> FilterWidgetsByPermission(List<WidgetDefinition> widgets)
        {
            ...
            {
                var allGranted = true;
                foreach (var permission in widget.Permissions)
                {
        ...
        private async Task<List<WidgetDefinition>> FilterWidgetsByMultiTenancySideAsync(List<WidgetDefinition> widgets)
        {
            var multiTenancySide = await AbpSession.GetMultiTenancySideAsync();
            return widgets.Where(w => w.Side.HasFlag(multiTenancySide)).ToList();
        }
