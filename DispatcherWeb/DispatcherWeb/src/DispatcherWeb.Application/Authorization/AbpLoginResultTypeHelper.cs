    ...
    public class AbpLoginResultTypeHelper : DispatcherWebServiceBase, ITransientDependency
    {
        public Exception CreateExceptionForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName)
        {
            switch (result)
            {
                ...
                case AbpLoginResultType.InvalidPassword:
                    var invalidUserNameOrPasswordMessage = tenancyName == null ? L("InvalidUserNameOrPassword_Detail") : L("InvalidUserNameOrPassword");
                    return new UserFriendlyException(L("LoginFailed"), invalidUserNameOrPasswordMessage);
        ...
        public string CreateLocalizedMessageForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName)
        {
            switch (result)
            {
                ...
                case AbpLoginResultType.InvalidUserNameOrEmailAddress:
                case AbpLoginResultType.InvalidPassword:
                    return tenancyName == null ? L("InvalidUserNameOrPassword_Detail") : L("InvalidUserNameOrPassword");
                case AbpLoginResultType.InvalidTenancyName:
