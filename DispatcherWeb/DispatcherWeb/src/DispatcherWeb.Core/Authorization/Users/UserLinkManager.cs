...
namespace DispatcherWeb.Authorization.Users
{
    public class UserLinkManager : DispatcherWebDomainServiceBase, IUserLinkManager
    {
        ...
        public virtual async Task<bool> AreUsersLinked(UserIdentifier firstUserIdentifier, UserIdentifier secondUserIdentifier)
        {
            ...
            {
                var firstUserLinkId = await GetUserLinkId(firstUserIdentifier);
                var secondUserLinkId = await GetUserLinkId(secondUserIdentifier);

                if (!firstUserLinkId.HasValue || !secondUserLinkId.HasValue)
                {
                    return false;
                }

                return firstUserLinkId == secondUserLinkId;
            });
        }
        ...
        public virtual async Task<long?> GetUserLinkId(UserIdentifier userIdentifier)
        {
            return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                return await (await _userAccountRepository.GetQueryAsync())
                    .Where(ua => ua.TenantId == userIdentifier.TenantId && ua.UserId == userIdentifier.UserId)
                    .Select(x => x.UserLinkId)
                    .FirstOrDefaultAsync();
            });
        }
