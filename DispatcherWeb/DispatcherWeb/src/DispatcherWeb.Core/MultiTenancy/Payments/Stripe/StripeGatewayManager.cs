...
namespace DispatcherWeb.MultiTenancy.Payments.Stripe
{
        ...
        ITransientDependency
    {
        ...
        public async Task UpdateSubscription(int newEditionId, int tenantId, bool isProrateCharged = false)
        {
            ...
            {
                var edition = (SubscribableEdition)await _editionManager.GetByIdAsync(newEditionId);
                newPlanId = GetPlanId(edition.Name, SubscriptionPayment.GetPaymentPeriodType(lastPayment.DayCount));
        ...
        public async Task HandleEventAsync(RecurringPaymentsDisabledEventData eventData)
        {
            var subscriptionPayment = await GetLastCompletedSubscriptionPaymentAsync(eventData.TenantId);

            int daysUntilDue;

            using (CurrentUnitOfWork.SetTenantId(null))
            {
                var edition = (SubscribableEdition)await _editionManager.GetByIdAsync(eventData.EditionId);
                daysUntilDue = edition.WaitingDayAfterExpire ?? 3;
            }

            var subscriptionService = new SubscriptionService();
            await subscriptionService.UpdateAsync(subscriptionPayment.ExternalPaymentId, new SubscriptionUpdateOptions
            {
                CancelAtPeriodEnd = false,
        ...
        public async Task HandleEventAsync(RecurringPaymentsEnabledEventData eventData)
        {
            var subscriptionPayment = await GetLastCompletedSubscriptionPaymentAsync(eventData.TenantId);
        ...
        public async Task HandleEventAsync(TenantEditionChangedEventData eventData)
        {
            ...
            {
                await CancelSubscription(subscriptionPayment.ExternalPaymentId);
                return;
            }
            ...
            using (CurrentUnitOfWork.SetTenantId(null))
            {
                var edition = (SubscribableEdition)await _editionManager.GetByIdAsync(eventData.NewEditionId.Value);
            ...
            if (!await DoesPlanExistAsync(newPlanId))
            {
                await CreatePlanAsync(newPlanId, newPlanAmount.Value, GetPlanInterval(subscriptionPayment.PaymentPeriodType), ProductName);
            }

            await UpdateSubscription(subscriptionPayment.ExternalPaymentId, newPlanId);
        }
        ...
        public async Task HandleInvoicePaymentSucceededAsync(Invoice invoice)
        {
            ...
            using (CurrentUnitOfWork.SetTenantId(null))
            {
                ...
                {
                    var lastPayment = await GetLastCompletedSubscriptionPaymentAsync(tenantId);
        ...
        private async Task<SubscriptionPayment> GetLastCompletedSubscriptionPaymentAsync(int tenantId)
        {
