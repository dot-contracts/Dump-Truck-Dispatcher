(function ($) {
    app.modals.CreateOrEditLeaseHaulerRequestModal = function () {
        ...
        var _insuranceExpiryAppService = abp.services.app.insurance;
        var _$form = null;
        ...
        this.init = function (modalManager) {
            ...
            $leaseHaulerDropdown.change(async function () {
                var leaseHaulerId = parseInt($leaseHaulerDropdown.val());
                if (leaseHaulerId) {
                    await checkInsuranceExpiry(leaseHaulerId);
                }
            ...
            async function checkInsuranceExpiry(leaseHaulerId) {
                const insurances = await _insuranceExpiryAppService.getActiveInsurances(leaseHaulerId);
                ...
                if (!insurances.length) {
                    ...
                    if (!isConfirmed) {
                        $leaseHaulerDropdown.val('').trigger('change');
                        throw new Error('Stopping execution because lease hauler does not have insurance');
                    }
                    return;
                }
                ...
                if (expiredInsurance) {
                    ...
                    if (!isConfirmed) {
                        $leaseHaulerDropdown.val('').trigger('change');
                        throw new Error('Stopping execution because lease hauler has expired insurance');
                    }
                }
            }
