(function ($) {
    app.modals.CreateOrEditCustomerNotificationModal = function () {
        var _modalManager;
        var _customerNotificationService = abp.services.app.customerNotification;
        var _$form = null;
        var _$startDateDropdown = null;
        var _$endDateDropdown = null;
        var _$editionsDropdown = null;
        var _$tenantsDropdown = null;
        var _$rolesDropdown = null;

        this.init = function (modalManager) {
            _modalManager = modalManager;

            _$form = _modalManager.getModal().find('form');
            _$form.validate();

            _$startDateDropdown = _$form.find("#StartDate");
            _$endDateDropdown = _$form.find("#EndDate");
            _$editionsDropdown = _$form.find("#EditionIds");
            _$tenantsDropdown = _$form.find("#TenantIds");
            _$rolesDropdown = _$form.find("#RoleNames");

            _$startDateDropdown.datepickerInit();

            _$endDateDropdown.datepickerInit();

            _$editionsDropdown.select2Init({
                abpServiceMethod: abp.services.app.edition.getEditionsSelectList,
                showAll: true,
                allowClear: true
            }).change(() => {
                _$tenantsDropdown.val(null).change();
            });

            _$tenantsDropdown.select2Init({
                abpServiceMethod: abp.services.app.tenant.getTenantsSelectList,
                abpServiceParamsGetter: (params) => ({
                    editionIds: _$editionsDropdown.val()
                }),
                showAll: false,
                allowClear: true
            });

            _$form.find("#Type").select2Init({
                showAll: true,
                allowClear: false
            });

            _$rolesDropdown.select2Init({
                showAll: true,
                allowClear: true
            });

        };

        this.save = function () {
            if (!_$form.valid()) {
                _$form.showValidateMessage();
                return;
            }

            var model = _$form.serializeFormWithMultipleToObject();
            model.EditionIds = _$editionsDropdown.val();
            model.TenantIds = _$tenantsDropdown.val();
            model.RoleNames = _$rolesDropdown.val();

            _modalManager.setBusy(true);
            _customerNotificationService.editCustomerNotification(model).done(function () {
                abp.notify.info('Saved successfully.');
                _modalManager.close();
                abp.event.trigger('app.createCustomerNotificationModalSaved');
            }).always(function () {
                _modalManager.setBusy(false);
            });
        };
    };
})(jQuery);
