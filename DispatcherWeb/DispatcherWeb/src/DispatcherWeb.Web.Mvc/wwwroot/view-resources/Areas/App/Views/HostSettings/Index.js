(function ($) {
    $(function () {
        ...
        $('#SaveAllSettingsButton').click(function () {
            ...
            if (!$('#ListCacheSettingsForm').valid()) {
                $('#ListCacheSettingsForm').showValidateMessage();
                return;
            }
            ...
            _hostSettingsService.updateAllSettings({
                ...
                listCache: getListCacheSettings(),
                security: {
        ...
        function getListCacheSettings() {
            const $form = $('#ListCacheSettingsForm');

            const listCacheSettings = {
                caches: [],
                globalCacheVersion: $form.find('#GlobalCacheVersion').val(),
            };

            const $cacheCards = $form.find('.list-cache-container .list-cache.card');

            $cacheCards.each(function() {
                const $card = $(this);
                const cacheName = $card.data('name');

                const cache = {
                    cacheName: cacheName,
                    backend: {
                        isEnabled: $card.find('input[name^="Caches[' + cacheName + '].Backend.IsEnabled"]').is(':checked'),
                        slidingExpirationTimeMinutes: parseInt($card.find('input[name^="Caches[' + cacheName + '].Backend.SlidingExpirationTimeMinutes"]').val())
                    },
                    frontend: {
                        isEnabled: $card.find('input[name^="Caches[' + cacheName + '].Frontend.IsEnabled"]').is(':checked'),
                        slidingExpirationTimeMinutes: parseInt($card.find('input[name^="Caches[' + cacheName + '].Frontend.SlidingExpirationTimeMinutes"]').val())
                    },
                };

                listCacheSettings.caches.push(cache);
            });

            return listCacheSettings;
        }
    });
})(jQuery);
